...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        if (checkUser) return res.json({ success: false, message: 'Regisration Failed! User Already Exists' })
...
const loginUser = async (req, res) => {
    ...
    try {
        const checkUser = await User.findOne({ email })
        if (!checkUser) return res.json({ success: false, message: 'Account does not exist' })
